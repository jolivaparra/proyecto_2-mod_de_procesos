...
for i=1:size(perfil_pert, 1)
    ...
    if i==1
        for j=1:size(p.K_p , 2)
            ...
            salida_sin_limite = p.K_p(j) * (p.ref - y_lazo_cerrado(:,1)) + p.offset;
            ...
            graficos(nombre_graf, t_lazo_cerrado, T_p_lazo_cerrado, plot_temp_amb, ...
                    plot_irr_sol, salida_vent, salida_bomb);
        ...
        salida_sin_limite = p.K_p(1) * (p.ref - y_lazo_cerrado(:,1)) + p.offset;
        salida_vent = min(p.V_vent_MAX, max(salida_sin_limite, p.V_MIN));
        salida_bomb = min(p.V_bomb_suficiente, max(salida_sin_limite, p.V_MIN));

        graficos(nombre_perfiles(i), t_lazo_cerrado, T_p_lazo_cerrado, plot_temp_amb, ...
                    plot_irr_sol, salida_vent, salida_bomb);
